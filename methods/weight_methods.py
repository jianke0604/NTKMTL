...
class PIVRG(WeightMethod):
    ...
    def backward(
        ...
        GTG, w = self.get_weighted_loss(losses, shared_parameters, task_specific_parameters, last_shared_parameters, representation, scores)
        if self.max_norm > 0:
            torch.nn.utils.clip_grad_norm_(shared_parameters, self.max_norm)
        return None, {"GTG": GTG, "weights": w}  # NOTE: to align with all other weight methods
...
class NTKMTL(WeightMethod):
    def __init__(self, n_tasks, device: torch.device, max_norm=1.0, ntk_exp=0.5):
        super().__init__(n_tasks, device=device)
        self.max_norm = max_norm
    ...
    def get_weighted_loss(
        ...
        **kwargs,
        ...
        params_per_task = len(task_specific_parameters) // self.n_tasks
        
        # store gradients of shared parameters for NTK computation
        shared_grads = torch.zeros(sum(grad_dims), self.n_tasks, self.batch_size).to(self.device)
        
        # store task-specific gradients: list of [batch_size] lists, each containing gradients for that task
        task_specific_grads = []
        for i in range(self.n_tasks):
            task_grads = []
            for j in range(self.batch_size):
                task_grads.append([])
            task_specific_grads.append(task_grads)

        
        # NTKMTL-SR 
        # need to comment out the backward() function
        ...
        # z_list = [representation for _ in range(self.n_tasks)]
        # self.batch_size = losses.shape[0]
        # lambda_ = np.array([0.0] * self.n_tasks)
        # J_full = []
        # for i in range(self.n_tasks):
        #     lambda_[i] = 0.0
        #     J = []
        #     for j in range(self.batch_size):
        #         delta = torch.autograd.grad(losses[j][i], z_list[i], retain_graph=True)
        #         flatten_grad = torch.cat([torch.flatten(grad) for grad in delta])
        #         J.append(flatten_grad)
        #         J_full.append(flatten_grad)
        #     J = torch.stack(J)
        #     JJ = J.mm(J.t())  # [batch_size, batch_size]
        #     lambda_[i] = torch.linalg.eigvalsh(JJ)[-1]
        # lambda_sum = np.sum(lambda_)
        # lambda_ = np.power(lambda_sum/lambda_, self.ntk_exp)  # [num_tasks, ]
        # loss = torch.mean(losses, 0)
        # loss = torch.sum(loss * torch.tensor(lambda_).to(self.device))
        # return loss, dict(weights=lambda_)
        
        
        # NTKMTL (n=1)
        # The NTK matrix of each task degenerates into a scalar, and the gradient is computed once for each task, which is fair to other gradient-oriented methods.
        grad_dims = []
        for param in shared_parameters:
            grad_dims.append(param.data.numel())
        grads = torch.Tensor(sum(grad_dims), self.n_tasks).to(self.device)
        for i in range(self.n_tasks):
            ...
            for p in shared_parameters:
                p.grad = None
        g, w_cpu = self.ntkgrad(grads)
        self.overwrite_grad(shared_parameters, g, grad_dims)
        return w_cpu
    

        # NTKMTL (n>1) just for reference, not used in experiments
        ...
        #         # perform backward
        #         retain_graph = not (i == self.n_tasks - 1 and j == self.batch_size - 1)
        #         losses[j][i].backward(retain_graph=retain_graph)
                
        #         # collect shared parameter gradients
        #         grad_vec = []
        #         for param in shared_parameters:
        #             if param.grad is not None:
        #                 grad_vec.append(torch.flatten(param.grad))
        #             else:
        #                 grad_vec.append(torch.zeros(param.numel()).to(self.device))
        #         shared_grads[:, i, j] = torch.cat(grad_vec)
                
        #         # collect task-specific parameter gradients
        #         theta_i = task_specific_parameters[i * params_per_task: (i + 1) * params_per_task]
        #         for k, param in enumerate(theta_i):
        #             if param.grad is not None:
        #                 task_specific_grads[i][j].append(param.grad.data.clone())
        #             else:
        #                 task_specific_grads[i][j].append(torch.zeros_like(param.data))
        
        # # compute NTK weights
        # lambda_ = np.array([0.0] * self.n_tasks)
        # for i in range(self.n_tasks):
        #     J = shared_grads[:, i, :].t()  # [batch_size, num_params]
        #     JJ = J.mm(J.t())  # [batch_size, batch_size]
        #     lambda_[i] = torch.linalg.eigvalsh(JJ)[-1]  # largest eigenvalue
        
        # lambda_sum = np.sum(lambda_)
        # lambda_ = np.power(lambda_sum / lambda_, self.ntk_exp)  # [num_tasks, ]
        
        # # weight shared parameter gradients
        # grads_avg = shared_grads.mean(2)  # [num_params, num_tasks]
        # g = grads_avg @ torch.tensor(lambda_, dtype=torch.float32).to(self.device)
        # self.overwrite_grad(shared_parameters, g, grad_dims)
        
        # # average task-specific parameter gradients
        # for i in range(self.n_tasks):
        #     theta_i = task_specific_parameters[i * params_per_task: (i + 1) * params_per_task]
        #     for k, param in enumerate(theta_i):
        #         avg_grad = torch.stack([task_specific_grads[i][j][k] for j in range(self.batch_size)]).mean(0)
        #         param.grad = avg_grad
        
        # return lambda_


    def ntkgrad(self, grads):
        GTG = grads.t().mm(grads) # [num_tasks, num_tasks]
        ntk_trace = torch.trace(GTG)
        # ntk_diagonal = GTG.diagonal()
        w_cpu = ntk_trace / GTG.diagonal() # [num_tasks]
        w_cpu = torch.pow(w_cpu, self.ntk_exp)

        g = grads @ w_cpu
        return g, w_cpu.cpu().numpy()
    ...
    def backward(
        ...
        w = self.get_weighted_loss(losses, shared_parameters, task_specific_parameters, last_shared_parameters, representation)
        if self.max_norm > 0:
            torch.nn.utils.clip_grad_norm_(shared_parameters, self.max_norm)
        return None, {"weights": w}  # NOTE: to align with all other weight methods
