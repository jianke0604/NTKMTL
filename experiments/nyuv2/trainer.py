...
from experiments.nyuv2.utils import ConfMatrix, delta_fn, depth_error, normal_error, delta_fn_arr
...
#     # binary mask to exclude undefined pixel space
#     binary_mask = (torch.sum(x_output, dim=1) != 0).float().unsqueeze(1).to(device)

#     if task_type == "semantic":
#         # semantic loss: depth-wise cross entropy
#         # Calculate loss for each sample in the batch, ignore undefined pixels (with ignore_index=-1)
#         loss = F.nll_loss(x_pred, x_output, ignore_index=-1, reduction='none')  # (batch_size, height, width)

#         # Now you can reduce the loss for each sample (e.g., sum or mean across spatial dimensions)
#         loss = loss.view(x_pred.size(0), -1)  # Flatten height and width dimensions
#         loss = loss.mean(dim=1)  

#     elif task_type == "depth":
#         # depth loss: L1 norm
#         # Calculate L1 loss for each pixel and average over non-zero (valid) pixels
#         loss = torch.abs(x_pred - x_output) * binary_mask
#         loss = torch.sum(loss, dim=[1, 2, 3]) / torch.sum(binary_mask, dim=[1, 2, 3])  # (batch_size,)

#     elif task_type == "normal":
#         # normal loss: dot product
#         # Compute dot product for each sample in the batch
#         loss = 1 - torch.sum((x_pred * x_output) * binary_mask, dim=[1, 2, 3]) / torch.sum(binary_mask, dim=[1, 2, 3])  # (batch_size,)
#     # print(task_type, loss.shape)
#     return loss


def main(path, lr, bs, device, ckpt_path=None):
    # ----
    # Nets
    # ---
    ...
    if ckpt_path is not None and os.path.isfile(ckpt_path):
        ...
        logging.info("No checkpoint provided or checkpoint not found. Training from scratch.")
    ...
    for epoch in epoch_iter:
        ...
        for j, batch in enumerate(train_loader):
            ...
            #         calc_loss(train_pred[2], train_normal, "normal"),
            #     ),
            #     dim=1  # Stack along the second dimension to create shape (batch_size, num_tasks)
            # )
            # losses = torch.mean(losses, dim=0).unsqueeze(0)  # Now losses has shape (1, num_tasks)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--wandb_logger_name", type=str, default="NYUv2", help="Name of Weights & Biases Logger.")
    ...
    main(path=args.data_path, lr=args.lr, bs=args.batch_size, device=device, ckpt_path=args.ckpt)
