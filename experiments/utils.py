...
common_parser.add_argument(
    ...
    type=float,
    default=2.0,
    help="performance-informed weight bound for PIVRG",
)
common_parser.add_argument(
    "--mintemp",
    type=float,
    default=10,
    help="minimum temperature for PIVRG",
)
...
common_parser.add_argument(
    ...
    type=float,
    default=1.0,
    help="lambda for consmtl",
)

# go4align
common_parser.add_argument('--robust_step_size', default=0.0001, type=float, help='for go4align')
common_parser.add_argument('--num_groups', default=5, type=int, help='number of groups')

# ntkmtl
common_parser.add_argument('--ntk_exp', type=float, default=0.5, help='exponent for NTKMTL')
...
def extract_weight_method_parameters_from_args(args):
    ...
    weight_methods_parameters.update(
        dict(
            ...
            pivrg=dict(bound=args.bound, mintemp=args.mintemp),
            consmtl=dict(lambda_=args.lambda_),
            go4align=dict(num_groups=args.num_groups, robust_step_size=args.robust_step_size),
            ntkmtl=dict(ntk_exp=args.ntk_exp, max_norm=args.max_norm),
        )
    )
